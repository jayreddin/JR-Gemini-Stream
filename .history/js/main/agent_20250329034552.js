...
export class GeminiAgent{
    ...
    async startCameraCapture() {
        ...
        try {
            ...
            const captureAndSend = async () => {
                if (!this.cameraInterval) return; // Stop if interval was cleared

                const now = performance.now();
                // Only capture if enough time has passed (throttle)
                if (now - lastCaptureTime >= this.captureInterval) {
                    lastCaptureTime = now;
            ...
            this.cameraInterval = true; // Use as a flag
            captureAndSend();
    ...
    async stopCameraCapture() {
        // Set flag to false to stop the requestAnimationFrame loop
        this.cameraInterval = false;
    ...
    async startScreenShare() {
        ...
        try {
            ...
            let lastCaptureTime = 0;
            const captureAndSend = async () => {
                if (!this.screenInterval) return; // Stop if flag was set to false
            ...
            // Start the capture loop
            this.screenInterval = true; // Use as a flag
            captureAndSend();
    ...
    async disconnect() {
        try {
            // Cancel any pending animation frames by setting flags to false
            this.cameraInterval = false;
            this.screenInterval = false;
