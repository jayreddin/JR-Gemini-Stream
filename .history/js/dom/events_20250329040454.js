...
/**
 * Updates the model banner with the current active model
 */
export const updateModelBanner = () => {
    const modelType = localStorage.getItem('modelType') || 'gemini-1.5-pro-latest'; // Updated fallback
    const displayName = modelType.replace('models/', ''); // Remove 'models/' prefix if present
    elements.modelBanner.textContent = `Model: ${displayName}`;
    elements.modelBanner.style.display = 'block';
};
...
export function setupEventListeners(agent) {
    ...
    elements.newChatBtn.addEventListener('click', async () => {
        try {
            ...
            // Ensure the agent is ready for a new conversation
            await ensureAgentReady(agent);
            
            // Optional: Start with a welcome message
            const welcomeMessage = document.createElement('div');
            welcomeMessage.className = 'chat-message model-message';
            welcomeMessage.textContent = 'How can I help you today?';
            elements.chatHistory.appendChild(welcomeMessage);
            
            console.info('Started new chat');
        } catch (error) {
